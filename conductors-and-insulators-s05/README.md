An Interactive Learning Website for Conductors and Insulators
📖 Project Overview
ConductoVerse is an educational web application designed to help school students learn about 
Conductors and Insulators through interactive lessons, experiments, quizzes, and virtual activities.
The project aims to make science learning more engaging and easier to understand by providing
practical examples and hands-on activities.

🌐 Live Demo
Explore the deployed website here:
[https://Condutoverse.vercel.app/](https://conductors-and-insulators-s05.vercel.app/)

🎯 Objectives
•Understand the concept of conductors and insulators.
• Learn real-life applications of conductive and insulating materials.
• Perform virtual experiments.
• Test knowledge through quizzes.
• Encourage interactive learning.
• Generate a completion certificate after learning.
🛠 Technologies Used
• HTML5
• CSS3
• JavaScript
• VS Code
• GitHub

📂 Project Structure
CONDUCTORS AND INSULATORS
│
├── css
│   └── style.css
│
├── js
│   ├── bulb.js
│   ├── quiz.js
│   └── uses.js
│
├── index.html
├── learn.html

├── bulb.html
├── quiz.html
├── uses.html
├── lab.html
├── certificate.html
│
└── README.md
✨ Features
🏠 Home Page
• Project introduction
• Navigation cards
• Easy access to all modules

📚 Learn Module
• Introduction to Conductors
• Introduction to Insulators
• Comparison Table
• Real-Life Examples
• Fun Facts

💡 Virtual Bulb Experiment
• Interactive material selection
• Bulb glows for correct conductor
• Error message for incorrect choice

🎯 Quiz Module
• Multiple-choice questions
• Automatic score calculation
• Performance badges

🌍 Real-Life Applications
• House Wiring
• Power Lines
• Chargers
• Safety Gloves
• Switch Boards

🧪 Virtual Lab
• Drag and Drop Activity
• Sort Conductors and Insulators
• Score Tracking

�
� Certificate Generation
• Student Name Input
• Dynamic Certificate Creation
• Achievement Recognition

🚀 Website Flow
Home Page
↓
Learn Module
↓
Bulb Experiment
↓
Quiz Module
↓
Real-Life Applications
↓
Virtual Lab
↓
Certificate
👥 Team Members & Responsibilities
Member 1 
• Storyboard Design
• Frontend Design
• Initial Prototype Development
• Project Planning

Member 2 
• Frontend Development
• Storyboard Support and Refinement
• Initial Prototype Development
• UI Implementation

Member 3 
• Deployment Research
• Technology Research and Analysis
• Backend Planning
• Project Groundwork

Member 4 
• Deployment Setup Research
• Backend Architecture Planning
• Integration Planning
• Technical Groundwork

📌 GitHub Issues
• Create Home Page
• Create Learn Module
• Develop Bulb Experiment
• Develop Quiz Module
• Create Uses Page
• Develop Virtual Lab
• Create Certificate Module
• Apply Styling
• Testing and Debugging
• Documentation

🎓 Expected Outcome
After completing the website activities, students will be able to:
• Identify Conductors and Insulators.
• Explain their properties.
• Recognize real-life applications.
• Perform simple virtual experiments.
• Successfully complete quizzes and activities.
• Earn a completion certificate.

🌟 Future Enhancements
• Audio-based explanations
• More virtual experiments
• Advanced quizzes
• Leaderboard system
• Student progress tracking
• Multi-language support

�
� License
This project is developed for educational purposes as part of an academic learning activity.
Team Name: SynapseX
Project Title:
ConductoVerse – Interactive Learning Website for Conductors and Insulators
